...
class MockSessionManager:
    ...
    def update_session_stats(self, session_id, temperature_data):
        if session_id in self.sessions:
            ...
            print(f"Updated stats for session {session_id}: max={session['max_temperature']}, min={session['min_temperature']}")
...
class TestSessionTracker:
    ...
    def process_temperature_reading(self, device_id, temperature, timestamp=None, user_id=None):
        ...
        print(f"Processed reading: device={device_id}, temp={temperature}°F, active={device_id in self.active_session_devices}")
...
def test_basic_session_tracking():
    ...
    for session in completed_sessions:
        print(f"Session {session['id']}: {session['session_type']}, duration: {session.get('duration_minutes', 'N/A')} min, max_temp: {session.get('max_temperature', 'N/A')}°F")
