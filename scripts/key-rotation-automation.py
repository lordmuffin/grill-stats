...
class KeyRotationScheduler:
    ...
    def start(self):
        ...
        # Log service start
        self._log_event(KeyRotationEvent(
            timestamp=datetime.now(timezone.utc).isoformat(),
...
def main():
    ...
    parser.add_argument('--config', '-c', default='/etc/grill-stats/key-rotation.json',
                       help='Configuration file path')
    parser.add_argument('--rotate', '-r', action='store_true',
                       help='Perform immediate key rotation')
    parser.add_argument('--status', '-s', action='store_true',
                       help='Show current status')
    parser.add_argument('--health-check', '-h', action='store_true',
