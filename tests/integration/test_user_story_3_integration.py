        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test("setup_authentication", "PASS", "User authenticated successfully")
                    return True
            
            self.log_test("setup_authentication", "FAIL", f"Authentication failed: {response.text}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    test_device = next((d for d in devices if d.get("device_id") == TEST_DEVICE_ID), None)
                    if test_device:
                        self.log_test("test_device_list_access", "PASS", f"Found {len(devices)} devices including test device")
                        return True
                    else:
                        self.log_test("test_device_list_access", "FAIL", f"Test device {TEST_DEVICE_ID} not found")
                        return False
            
            self.log_test("test_device_list_access", "FAIL", f"Device list request failed: {response.text}")
            return False
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    if not missing_fields:
                        ...
                        self.log_test("test_live_device_data_endpoint", "PASS", 
                                    f"Live data retrieved: {len(channels)} channels, battery: {status.get('battery_level', 'N/A')}%")
                        return True
                    else:
                        self.log_test("test_live_device_data_endpoint", "FAIL", 
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    if channels:
                        ...
                        if not missing_fields:
                            self.log_test("test_device_channels_endpoint", "PASS", 
                                        f"Channel configuration retrieved: {len(channels)} channels")
                            return True
                        else:
                            self.log_test("test_device_channels_endpoint", "FAIL", 
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    required_fields = ["device_id", "battery_level", "signal_strength", "connection_status"]
                    ...
                    if not missing_fields:
                        ...
                        self.log_test("test_device_status_endpoint", "PASS", 
                                    f"Device status retrieved: battery={battery}%, signal={signal}%, status={connection}")
                        return True
                    else:
                        self.log_test("test_device_status_endpoint", "FAIL", 
    ...
    def test_live_data_stream(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                    ...
                    if event.data:
                        try:
                            ...
                            if data.get("device_id") == TEST_DEVICE_ID and "channels" in data:
                                self.log_test("test_live_data_stream", "PASS", 
                ...
                if messages:
                    self.log_test("test_live_data_stream", "PARTIAL", 
                                f"SSE stream connected but no valid data: {len(messages)} messages")
                else:
                    self.log_test("test_live_data_stream", "FAIL", "No messages received from SSE stream")
                return False
            
            self.log_test("test_live_data_stream", "FAIL", 
    ...
    def test_database_schema(self) -> bool:
        ...
        try:
            ...
            schema_file = os.path.join(
                os.path.dirname(__file__), '..', '..', 'database-init', 'live-data-schema.sql'
            )
            ...
            if os.path.exists(schema_file):
                ...
                if not missing_tables:
                    self.log_test("test_database_schema", "PASS", 
                                f"Database schema file contains all required tables")
                    return True
                else:
                    self.log_test("test_database_schema", "FAIL", 
            ...
            self.log_test("test_database_schema", "FAIL", 
                         f"Database schema test error: {str(e)}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "live" in content.lower() or "dashboard" in content.lower():
                    self.log_test("test_frontend_routing", "PASS", 
                                "Frontend live dashboard route accessible")
                    return True
                else:
                    self.log_test("test_frontend_routing", "PARTIAL", 
                                "Frontend route accessible but content unclear")
                    return False
            else:
                self.log_test("test_frontend_routing", "FAIL", 
                             f"Frontend route not accessible: {response.status_code}")
                return False
                
        except Exception as e:
            self.log_test("test_frontend_routing", "SKIP", 
                         f"Frontend test skipped: {str(e)}")
        ...
        try:
            ...
            if response.status_code in [400, 404, 500]:
                ...
                if data.get("status") == "error":
                    self.log_test("test_error_handling", "PASS", 
                                "Error handling works for invalid device ID")
                    return True
            
            self.log_test("test_error_handling", "FAIL", 
    ...
    def test_performance(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200 and response_time < 2.0:
                self.log_test("test_performance", "PASS", 
                            f"Live data endpoint response time: {response_time:.2f}s")
                return True
            elif response.status_code == 200:
                self.log_test("test_performance", "FAIL", 
