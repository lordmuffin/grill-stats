...
from auth.jwt_middleware import JWTManager, get_current_user, jwt_required
from security.rate_limiter import RateLimit, RateLimitAlgorithm, RateLimiter
from security.security_middleware import SecurityConfig, SecurityMiddleware
from security.waf import WAF, ActionType, ThreatLevel, WAFRule
...
class TestRateLimiting:
    ...
    def rate_limiter(self):
        ...
        limiter = RateLimiter()
        limiter.add_limit("test", RateLimit(requests=5, window=60, algorithm=RateLimitAlgorithm.FIXED_WINDOW))
        return limiter
    ...
    def test_rate_limit_algorithms(self):
        ...
        # Test token bucket
        limiter.add_limit(
            "token_bucket", RateLimit(requests=10, window=60, algorithm=RateLimitAlgorithm.TOKEN_BUCKET, burst=5)
        )

        # Test sliding window
        limiter.add_limit("sliding_window", RateLimit(requests=10, window=60, algorithm=RateLimitAlgorithm.SLIDING_WINDOW))
...
class TestSecurityMiddleware:
    ...
    def test_json_validation(self):
        ...
        with pytest.raises(Exception):
            validator.validate_json({"level1": {"level2": {"level3": {"level4": "too deep"}}}})
    ...
    def test_file_upload_validation(self):
        ...
        # Test valid file
        valid_file = FileStorage(stream=BytesIO(b"test content"), filename="test.txt", content_type="text/plain")
        assert validator.validate_file_upload(valid_file)

        # Test invalid extension
        invalid_file = FileStorage(
            stream=BytesIO(b"test content"), filename="test.exe", content_type="application/octet-stream"
        )
        with pytest.raises(Exception):
