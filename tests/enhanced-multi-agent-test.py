    ...
    if response.status_code == 200:
        try:
            ...
            if overall_status == 'healthy':
                ...
                return True, 'degraded', 'Service operational, dependencies unavailable (expected in test)'
            else:
        ...
        try:
            ...
            if any(expected in error_msg for expected in expected_errors):
                return True, 'degraded', f'Expected dependency error: {data.get("error", "Unknown")}'
            else:
                return False, 'error', f'Unexpected error: {data.get("error", "Unknown")}'
                
        except json.JSONDecodeError:
            return False, 'invalid', f'Invalid error response (status: {response.status_code})'
...
class EnhancedContainerAgent:
    ...
    def __init__(self, name: str, service_path: str, container_name: str, port: int, service_type: str):
        self.name = name
...
class EnhancedMultiAgentTestOrchestrator:
    ...
    def generate_enhanced_report(self) -> Dict:
        ...
        for result in self.results:
            ...
            report['summary']['health_status_distribution'][status] = report['summary']['health_status_distribution'].get(status, 0) + 1
    ...
    def print_enhanced_report(self):
        ...
        for status, count in summary['health_status_distribution'].items():
            emoji = {'healthy': '💚', 'degraded': '⚠️', 'error': '❌', 'invalid': '🔴'}.get(status, '❓')
        ...
        for agent in report['agents']:
            status_emoji = {'healthy': '💚', 'degraded': '⚠️', 'error': '❌', 'invalid': '🔴'}.get(agent['health_status'], '❓')
            
            print(f"\n🤖 {agent['agent_name']} ({agent['service_name']})")
            print(f"   Container: {agent['container_name']}")
            print(f"   Build: {agent['status']['build']} | Start: {agent['status']['start']} | Health: {agent['status']['health']}")
