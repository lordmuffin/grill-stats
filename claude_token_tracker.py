...
def display_usage_info(api_key, model, rate_limit_info, verbose):
    ...
    if has_rich:
        console.print(Panel(f"[bold blue]Claude Pro Token Usage Tracker[/bold blue]", 
                        box=box.ROUNDED))
    else:
        ...
        for key, value in rate_limit_info.items():
            if 'reset' in key:
                try:
                    ...
                    console.print(f"[bold]{key.replace('anthropic-ratelimit-', '').replace('-', ' ').title()}:[/bold] {reset_time.strftime('%Y-%m-%d %H:%M:%S')} (in {minutes}m {seconds}s)")
