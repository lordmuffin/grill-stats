...
def get_temperature_stats(device_id):
    ...
    with tracer.start_as_current_span("get_temperature_stats"):
        try:
            ...
            logger.error(
                "Failed to get temperature statistics",
                device_id=device_id,
                error=str(e),
            )
            return jsonify({"status": "error", "message": str(e)}), 500
...
def get_device_live_data(device_id):
    ...
    with tracer.start_as_current_span("get_device_live_data"):
        try:
            ...
            logger.info(
                "Live device data retrieved",
                device_id=device_id,
                channels=len(live_data["channels"]),
            )
...
def get_device_live_data_internal(device_id):
    ...
    try:
        ...
        logger.error(
            "Failed to get live device data internally",
            device_id=device_id,
            error=str(e),
        )
        return None
