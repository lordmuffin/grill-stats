...
from ..schemas.events import (
    TemperatureValidatedEvent, AnomalyDetectedEvent, AlertTriggeredEvent,
    AnomalyDetails, AlertAction, SeverityLevel, BaseEvent
)
...
# Prometheus metrics
ANOMALIES_DETECTED = Counter('anomalies_detected_total', 'Total anomalies detected', ['device_id', 'anomaly_type'])
...
class AnomalyDetector:
    ...
    async def process_validated_reading(self, event: BaseEvent):
        ...
        try:
            ...
            if anomaly_result["is_anomaly"]:
                ...
                if anomaly_result["severity"] in [SeverityLevel.HIGH, SeverityLevel.CRITICAL]:
            ...
            logger.error("Failed to process validated reading", 
                        device_id=device_id,
                        error=str(e))
            raise
            ...
            if len(recent_features) > 1:
                ...
                if len(recent_temps) >= 2:
                    ...
                    if len(recent_temps) >= 3:
                        features["temp_second_derivative"] = (recent_temps[-1] - recent_temps[-2]) - (recent_temps[-2] - recent_temps[-3])
        ...
        try:
            ...
            for model_type, model in device_models.items():
                try:
                    ...
                    logger.warning(f"Model {model_type} prediction failed", device_id=device_id, error=str(e))
    ...
    async def _create_anomaly_event(self, original_event: TemperatureValidatedEvent, anomaly_result: Dict[str, Any]) -> AnomalyDetectedEvent:
    ...
    async def _get_historical_stats(self, device_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_anomaly": self.anomaly_history[device_id][-1]["timestamp"].isoformat() if self.anomaly_history.get(device_id) else None
            }
        ...
        for device_id in list(self.device_features.keys()):
            if len(self.device_features[device_id]) >= self.min_training_samples:
                try:
                    ...
                    logger.error("Failed to train models for device", device_id=device_id, error=str(e))
    ...
    async def _train_device_models(self, device_id: str):
        ...
        try:
            ...
            try:
                ...
                logger.warning("Failed to train Isolation Forest", device_id=device_id, error=str(e))
        ...
        try:
            ...
            for key in model_keys:
                ...
                try:
                    ...
                    logger.warning("Failed to load models for device", 
                                 device_id=device_id, 
                                 error=str(e))
        ...
        try:
            ...
            for device_id in list(self.anomaly_history.keys()):
                ...
                self.anomaly_history[device_id] = [
                    anomaly for anomaly in self.anomaly_history[device_id]
