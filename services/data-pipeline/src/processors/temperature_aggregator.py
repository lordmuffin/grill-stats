...
# Prometheus metrics
TEMPERATURE_READINGS_PROCESSED = Counter(
    "temperature_readings_processed_total",
    "Total temperature readings processed",
    ["device_id", "status"],
)
VALIDATION_DURATION = Histogram(
    "temperature_validation_duration_seconds",
...
class TemperatureAggregationService:
    ...
    async def process_temperature_reading(self, event: BaseEvent):
        ...
        if not isinstance(event, TemperatureReadingEvent):
            logger.warning(
                "Received non-temperature reading event",
        ...
        try:
            ...
            logger.error(
                "Failed to process temperature reading",
                device_id=device_id,
                error=str(e),
            )
    ...
    async def _validate_temperature_reading(self, event: TemperatureReadingEvent) -> Dict[str, Any]:
        ...
        if (
            temperature < self.validation_rules["temperature_range"]["min"]
            or temperature > self.validation_rules["temperature_range"]["max"]
        ):
            errors.append(
                ValidationError(
                    field="temperature",
