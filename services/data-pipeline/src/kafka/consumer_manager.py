...
from ..schemas.events import deserialize_event, BaseEvent
from ..utils.config import KafkaConfig

logger = structlog.get_logger()

# Prometheus metrics
MESSAGES_RECEIVED = Counter('kafka_messages_received_total', 'Total messages received from Kafka', ['topic', 'status'])
...
class ConsumerManager:
    ...
    def _create_consumer(self, group_id: str, topics: List[str]) -> Consumer:
        ...
        try:
            ...
            logger.error("Failed to create Kafka consumer", group_id=group_id, topics=topics, error=str(e))
            raise
    
    async def start_consumer(self, consumer_id: str, topics: List[str], message_handler: Callable[[BaseEvent], None], group_id: Optional[str] = None):
        ...
        try:
            ...
            logger.info("Consumer started", consumer_id=consumer_id, topics=topics, group_id=group_id)
            
        except Exception as e:
            logger.error("Failed to start consumer", consumer_id=consumer_id, topics=topics, error=str(e))
            raise
        ...
        try:
            while self.is_running:
                try:
                    ...
                    logger.error("Kafka exception in consumer loop", 
                               consumer_id=consumer_id, 
                               error=str(e))
                    await asyncio.sleep(1)
                    
                except Exception as e:
                    logger.error("Unexpected error in consumer loop", 
                               consumer_id=consumer_id, 
                               error=str(e))
                    await asyncio.sleep(1)
