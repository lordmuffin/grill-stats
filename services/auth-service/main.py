...
def login():
    ...
    try:
        ...
        if not check_rate_limit(email, ip_address):
            ...
            return jsonify(error_response("Too many failed attempts. Please try again later.")), 429
...
def connect_thermoworks():
    ...
    try:
        ...
        if not credential_manager:
            return jsonify(error_response("Credential encryption service unavailable")), 503
        ...
        from thermoworks_client import ThermoworksClient, ThermoworksAuthenticationError
        ...
        try:
            ...
            if auth_response and auth_response.get('access_token'):
                ...
                if not success:
                    return jsonify(error_response("Failed to store encrypted credentials")), 500
...
def get_thermoworks_credentials():
    ...
    try:
        ...
        if not email or not password:
            return jsonify(error_response("No ThermoWorks credentials found for user")), 404
...
def get_thermoworks_status():
    ...
    try:
        ...
        return jsonify(success_response({
            ...
            'last_validated': credential_info['last_validated'].isoformat() if credential_info['last_validated'] else None,
...
def disconnect_thermoworks():
    ...
    try:
        ...
        if not success:
            return jsonify(error_response("Failed to delete encrypted credentials")), 500
