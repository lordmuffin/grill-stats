...
from rfx_gateway_client import RFXGatewayClient, RFXGatewayError, GatewaySetupStep
...
def discover_gateways():
    ...
    try:
        ...
        return jsonify(error_response(f"Error discovering gateways: {str(e)}", 500)), 500
...
def connect_to_gateway(gateway_id):
    ...
    try:
        ...
        return jsonify(error_response(f"Error connecting to gateway: {str(e)}", 400)), 400
    except Exception as e:
        logger.error(f"Error connecting to gateway {gateway_id}: {e}")
        return jsonify(error_response(f"Error connecting to gateway: {str(e)}", 500)), 500
...
def scan_wifi_networks(gateway_id):
    ...
    try:
        ...
        return jsonify(error_response(f"Error scanning Wi-Fi networks: {str(e)}", 400)), 400
    except Exception as e:
        logger.error(f"Error scanning Wi-Fi networks for gateway {gateway_id}: {e}")
        return jsonify(error_response(f"Error scanning Wi-Fi networks: {str(e)}", 500)), 500
...
def configure_wifi(gateway_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify(error_response("Failed to configure Wi-Fi connection", 400)), 400
    except RFXGatewayError as e:
...
def link_to_account(gateway_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify(error_response("Failed to link gateway to ThermoWorks Cloud account", 400)), 400
    except RFXGatewayError as e:
        logger.error(f"Error linking gateway {gateway_id} to account: {e}")
        return jsonify(error_response(f"Error linking gateway to account: {str(e)}", 400)), 400
    except Exception as e:
        logger.error(f"Error linking gateway {gateway_id} to account: {e}")
        return jsonify(error_response(f"Error linking gateway to account: {str(e)}", 500)), 500
...
def get_setup_status(gateway_id):
    ...
    try:
        ...
        return jsonify(error_response(f"Error getting setup status: {str(e)}", 400)), 400
    except Exception as e:
        logger.error(f"Error getting setup status for gateway {gateway_id}: {e}")
        return jsonify(error_response(f"Error getting setup status: {str(e)}", 500)), 500
