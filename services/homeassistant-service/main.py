...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("/app/logs/homeassistant-service.log", "a")],
)
...
class HomeAssistantService:
    def __init__(self):
        ...
        self.ha_client = HomeAssistantClient(self.config, mock_mode=os.getenv("MOCK_MODE", "false").lower() == "true")

        self.entity_manager = EntityManager(self.ha_client, self.config.entity_prefix)
        ...
        self.discovery_service = DiscoveryService(self.ha_client, self.config)

        self.automation_helper = AutomationHelper(self.ha_client, self.entity_manager)
        ...
        self.health_monitor = HealthMonitor(self.ha_client, self.entity_manager, self.state_sync, self.discovery_service)
    ...
    def run(self):
        ...
        try:
            ...
            self.app.run(
                host="0.0.0.0", port=int(os.getenv("PORT", "5000")), debug=os.getenv("DEBUG", "false").lower() == "true"
            )
