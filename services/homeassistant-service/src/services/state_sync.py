...
from ..models.ha_models import HAEvent, HAStateChange
    ...
    async def force_sync_all(self) -> Dict[str, Any]:
        try:
            ...
            for entity_id, entity_state in all_entities.items():
                try:
                    success = self.ha_client.set_entity_state(entity_id, str(entity_state.state), entity_state.attributes)
        ...
        for entity_id, update_data in batch:
            try:
                success = self.ha_client.set_entity_state(
                    update_data["entity_id"], str(update_data["state"]), update_data["attributes"]
                )
                ...
                if success:
                    ...
                    self.entity_manager.update_entity_state(
                        update_data["entity_id"], update_data["state"], update_data["attributes"]
                    )
    ...
    async def _handle_ha_state_change(self, entity_id: str, event_data: Dict):
        try:
            ...
            if entity_id.startswith("sensor.grill_stats") or entity_id.startswith("binary_sensor.grill_stats"):
                ...
                if new_state:
                    self.entity_manager.update_entity_state(entity_id, new_state.get("state"), new_state.get("attributes", {}))
