...
class HealthMonitor:
    def __init__(self, ha_client, entity_manager, state_sync, discovery_service):
        ...
        self.health_checks = {
            "ha_connection": HealthCheck(
                ...
                check_function="check_ha_connection",
                interval_seconds=60,
                critical=True
            ),
            ...
            try:
                ...
                for check_name, check_config in self.health_checks.items():
                    ...
                    if (not last_check or 
                        (current_time - last_check).total_seconds() >= check_config.interval_seconds):
        ...
        for check_name in check_names:
            try:
                ...
                try:
                    ...
                    result = {
                        "status": "failed",
                        "message": f"Health check timed out after {check_config.timeout_seconds}s"
                    }
    ...
    async def check_entity_registry(self) -> Dict[str, Any]:
        try:
            ...
            if total_entities > 0:
                ...
                return {
                    "status": "warning",
                    "message": "No entities in registry",
                    "details": registry_stats
                }
        ...
        try:
            ...
            if sync_stats.get("successful_syncs", 0) + sync_stats.get("failed_syncs", 0) > 0:
                success_rate = (
                    ...
                    (sync_stats.get("successful_syncs", 0) + sync_stats.get("failed_syncs", 0)) * 100
                )
            ...
            return {
                "status": "error",
                "message": f"State sync check failed: {str(e)}",
                "details": {}
            }
    ...
    async def check_discovery_service(self) -> Dict[str, Any]:
        try:
            ...
            return {
                "status": "error",
                "message": f"Discovery service check failed: {str(e)}",
                "details": {}
            }
    ...
    def _get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "health_percentage": (
                status_counts["healthy"] / max(total_checks, 1) * 100
            )
        }
