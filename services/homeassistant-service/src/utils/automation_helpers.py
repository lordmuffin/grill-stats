...
from ..models.entity_models import AutomationAction, AutomationCondition, AutomationTrigger, HAAutomation
from ..models.ha_models import HAEvent, HAServiceCall
...
class AutomationHelper:
    ...
    def create_device_offline_automation(
        self, device_id: str, alert_name: str = "Device Offline", offline_duration: str = "00:05:00"  # 5 minutes
    ) -> str:
        try:
            ...
            trigger = AutomationTrigger(
                trigger_type="state", entity_id=entity_id, from_state="on", to_state="off", for_duration=offline_duration
            )
    ...
    def create_battery_low_automation(
        self, device_id: str, battery_threshold: int = 20, alert_name: str = "Low Battery"
    ) -> str:
        try:
            ...
            start_actions = [
                ...
                AutomationAction(
                    ...
                    service_data={
                        ...
                        "snapshot_entities": [
                            f"sensor.grill_stats_{device_id}_{probe}_temperature"
                            for device_id in device_ids
    ...
    async def deploy_automation_to_ha(self, automation_id: str) -> bool:
        try:
            ...
            service_call = HAServiceCall(domain="automation", service="reload", service_data={})
    ...
    def _convert_trigger_to_ha(self, trigger: AutomationTrigger) -> Dict[str, Any]:
        ha_trigger = {"platform": trigger.trigger_type, "entity_id": trigger.entity_id}
    ...
    def _convert_condition_to_ha(self, condition: AutomationCondition) -> Dict[str, Any]:
        ha_condition = {"condition": condition.condition_type, "entity_id": condition.entity_id}
    ...
    def _convert_action_to_ha(self, action: AutomationAction) -> Dict[str, Any]:
        if action.action_type == "call_service":
            return {"service": action.service, "data": action.service_data or {}, "target": action.target or {}}
        else:
...
class NotificationHelper:
    ...
    async def send_device_status_notification(self, device_id: str, status: str, channels: List[str] = None) -> bool:
        try:
            channels = channels or ["persistent"]
                ...
                if channel in self.notification_channels:
                    service_call = HAServiceCall(
                        ...
                        service_data={
                            "title": title,
                            "message": message,
                            "data": {"device_id": device_id, "status": status, "alert_type": "device_status"},
                        },
                    )
...
class SceneHelper:
        ...
        try:
            ...
            for device_id in device_ids:
                ...
                for entity in device_entities:
                    entities_state[entity.entity_id] = {"state": entity.state, "attributes": entity.attributes}

            # Create scene
            service_call = HAServiceCall(
                domain="scene",
                service="create",
                service_data={"scene_id": f"grill_{scene_name.lower().replace(' ', '_')}", "entities": entities_state},
            )
    ...
    async def activate_scene(self, scene_id: str) -> bool:
        try:
            service_call = HAServiceCall(domain="scene", service="turn_on", target={"entity_id": f"scene.{scene_id}"})
