...
from .rate_limiter import rate_limiter, get_rate_limit_key
...
class ThreatDetector:
    ...
    def track_suspicious_activity(self, ip: str, activity_type: str) -> bool:
        ...
        ip_data["activities"] = [
            activity for activity in ip_data["activities"]
            if now - activity["timestamp"] < 3600
        ]
        
        # Add new activity
        ip_data["activities"].append({
            "type": activity_type,
            "timestamp": now
        })
...
class SecurityMiddleware:
    ...
    def handle_large_payload(self, error):
        ...
        return jsonify({
            ...
            "message": f"Maximum content length is {self.config.MAX_CONTENT_LENGTH} bytes"
        }), 413
